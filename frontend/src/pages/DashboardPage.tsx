...
import { getLastTransactionDate, getSyncedTransactions, listUnsynced, uploadCsv } from '../api/transactions'
...
function SplitBadge({ splitType }: { splitType: SplitType | null }) {
  ...
  const intent =
    splitType === 'personal' || splitType === 'already_added' ? undefined :
    splitType === 'equal' ? 'primary' : 'none'
  return <Tag minimal intent={intent}>{SPLIT_LABELS[splitType]}</Tag>
}
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    listUnsynced().then(txs => setUnsyncedCount(txs.length)).catch(() => setUnsyncedCount(0))
    getLastTransactionDate().then(setLastDate).catch(() => setLastDate(null))
  }, [])
  ...
  return (
    <div>
      ...
      {(unsyncedCount ?? 0) > 0 && (
        ...
        >
          <span>
            <strong>{unsyncedCount}</strong> transaction{unsyncedCount !== 1 ? 's' : ''} waiting for review
          </span>
          <Button intent="warning" icon="arrow-right" onClick={() => navigate('/review')} style={{ marginLeft: 16 }}>
            Review &amp; push
          </Button>
        </Callout>
      )}
      ...
      <div className="dashboard-stats">
        <div className="stat-card">
          {unsyncedCount === null ? <Spinner size={28} /> : <span className="stat-number">{unsyncedCount}</span>}
          <span className="stat-label">Pending transactions</span>
        </div>
        <div className="stat-card">
          {lastDate === undefined ? <Spinner size={28} /> : <span className="stat-number stat-number--date">{lastDate ?? '—'}</span>}
      ...
      <Card className="upload-card">
        <div className="upload-card-body">
          ...
          <div style={{ flexShrink: 0 }}>
            ...
            <Button intent="primary" icon="upload" large loading={uploading} onClick={() => fileInputRef.current?.click()}>
              Choose CSV file
            </Button>
          </div>
        </div>
        ...
        {error && <p style={{ margin: '12px 0 0', color: '#c23030', fontSize: 13 }}>{error}</p>}
      </Card>
      ...
      <div style={{ marginTop: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
            <h4 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: '#1c2127' }}>Transaction history</h4>
            {historyTotal > 0 && <span style={{ fontSize: 12, color: '#738091' }}>{historyTotal} synced</span>}
          </div>
          {totalPages > 1 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <Button minimal small icon="chevron-right" disabled={page >= totalPages - 1} onClick={() => setPage(p => p + 1)} />
            </div>
          )}
        </div>

        <div style={{ background: '#fff', border: '1px solid #e1e8ed', borderRadius: 8, overflowX: 'auto', position: 'relative' }}>
          {historyLoading && (
                ...
                <tr key={tx.id}>
                  ...
                  <td>
                    ...
                    <div style={{ fontSize: 12, color: '#738091', marginTop: 2 }}>
                      {tx.merchant_key}{tx.sub_merchant_key ? ` · ${tx.sub_merchant_key}` : ''}
                    </div>
                  </td>
                  <td style={{ textAlign: 'right', fontWeight: 600, whiteSpace: 'nowrap' }}>
                    {fmt(parseFloat(tx.amount))}
                  </td>
                  <td style={{ fontSize: 13, color: '#738091' }}>{tx.card_member ?? '—'}</td>
                  <td><SplitBadge splitType={tx.split_type} /></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {totalPages > 1 && (
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 10, gap: 8, alignItems: 'center' }}>
            <Button minimal small icon="chevron-left" disabled={page === 0} onClick={() => setPage(p => p - 1)} />
            <span style={{ fontSize: 13, color: '#738091' }}>Page {page + 1} of {totalPages}</span>
            <Button minimal small icon="chevron-right" disabled={page >= totalPages - 1} onClick={() => setPage(p => p + 1)} />
          </div>
        )}
      </div>
